...
export function useFileImport() {
  ...
  const [originalIbmaps, setOriginalIbmaps] = useState<IbmapsState | null>(null);
    ...
    try {
      ...
      if (filename.toLowerCase().endsWith('.ibmaps')) {
        ...
        if (parseResult.warnings.length > 0) {
          ...
          // Optional: Expose warnings to UI if needed, for now just log
        }

        const workbook = rawSignalsToWorkbook(parseResult.signals, parseResult.devices);
        
        setRaw(workbook);
        setProtocols({
          internalProtocol: 'BACnet Server', // Fixed for this PoC
          externalProtocol: 'Modbus Master'  // Fixed for this PoC
        });
        setOriginalIbmaps({
          content: xmlContent,
