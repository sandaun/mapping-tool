 ...
 */
function createDataRow(s: RawSignal, idx: number, devices: Map<number, IbmapsDevice>): CellValue[] {
  const id = idx + 1; // 1-based index
  const dev = devices.get(s.modbus.deviceIndex);
  const deviceName = dev ? `RTU // Port A // ${dev.name}` : `RTU // Port A // Device ${s.modbus.deviceIndex}`;
...
function formatType(type: number): string {
  const types: Record<number, string> = {
    0: '0: AI', 1: '1: AO', 2: '2: AV', 3: '3: BI', 
    4: '4: BO', 5: '5: BV', 13: '13: MI', 14: '14: MO', 19: '19: MV'
  };
...
function formatReadFunc(func: number): string {
  ...
  const funcs: Record<number, string> = {
    1: '1: Read Coils', 2: '2: Read Discrete Inputs',
    3: '3: Read Holding Registers', 4: '4: Read Input Registers'
  };
...
function formatWriteFunc(func: number): string {
  if (func <= 0) return '-';
  const funcs: Record<number, string> = {
    5: '5: Write Single Coil', 6: '6: Write Single Register',
    15: '15: Write Multiple Coils', 16: '16: Write Multiple Registers'
  };
